...
if [[ $COMMAND = "start" ]]; then
  ...
  oc exec -ti $(oc get pods | grep $CONNECT_POD | awk '{print $1}') -- curl -X POST -H "Content-Type: application/json" --data '{"name": "FluentdSinkConnector", "config": {"connector.class":"org.fluentd.kafka.FluentdSinkConnector", "tasks.max":"1", "fluentd.connect":"localhost:24225", "fluentd.port":"24225", "fluentd.bind":"0.0.0.0", "fluentd.worker.pool.size":"1", "fluentd.counter.enabled":"true","fluentd.schemas.enable":"false", "topics":"output_tag", "fluentd.client.wait.until.buffer.flushed":"0", "fluentd.client.wait.until.flusher.terminated":"0", "fluentd.client.flush.interval":"10"}}' http://localhost:8083/connectors
  echo ""
